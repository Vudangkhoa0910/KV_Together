...
export default function UserNews() {
  ...
  const handleCreateNews = () => {
    router.push('/user/news/create');
  };

  const handleEditNews = (newsSlug: string) => {
    router.push(`/user/news/edit/${newsSlug}`);
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-2xl font-semibold text-gray-900">Bài viết của tôi</h1>
          <p className="mt-2 text-sm text-gray-700">
            Danh sách các bài viết mà bạn đã chia sẻ
          </p>
        </div>
            ...
            {news.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">
                  Chưa có bài viết nào. Hãy chia sẻ bài viết đầu tiên của bạn!
                </td>
              </tr>
            ) : (
              news.map((article) => (
